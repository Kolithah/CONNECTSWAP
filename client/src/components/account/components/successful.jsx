...
const successful = () => {
  ...
  if(isSuccess && data.filter((e)=>{return (e?.users?.some(user => user?.reqStatus == 6 ))})?.length==0){
    ...
    return <div className="flex flex-col w-full h-[300px] font-poppins text-2xl font-semibold justify-center align-middle items-center">No Successful Requests found </div>
  }
    else  return (
    <>
    ...
    <div className="flex  flex-col-reverse w-full h-full justify-center align-middle items-center">
      {data.filter((e)=>{return (e?.users?.some(user => user?.reqStatus == 6 ))})?.map((e)=>{
